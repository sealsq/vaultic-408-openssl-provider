...
static void *vic408_keymgmt_new(void *provctx)
{
...
static void vic408_keymgmt_free(void *keydata)
{
...
static void *vic408_keymgmt_load(const void *reference, size_t reference_sz)
{
    sealsq_VIC408_Prov_Print(LOG_DBG_ON, "Enter - %s ", __FUNCTION__);
    ...
    *(sealsq_VIC408_provider_store_obj_t **)reference = NULL;
    ...
    sealsq_VIC408_Prov_Print(LOG_DBG_ON, "PubKey len = %d", pStoreCtx->pub_key_len);
    return pStoreCtx;
}

static int vic408_keymgmt_has(const void *keydata, int selection)
{
    sealsq_VIC408_Prov_Print(LOG_DBG_ON, "Enter - %s ", __FUNCTION__);
    ...
    if (selection == OSSL_KEYMGMT_SELECT_PRIVATE_KEY)
    {
        ...
        {
            sealsq_VIC408_Prov_Print(LOG_DBG_ON, "Possible correct Private Key at key index %x", key->privkey_index);
            return 1;
        }
        else
            return 0;
...
static int vic408_keymgmt_import(void *keydata, int selection, OSSL_PARAM params[])
{
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
    {
        ...
        if (param != NULL)
        {
            ...
            if (!OSSL_PARAM_get_octet_string(param, (void **)&pubkey, 0, &usedlen))
            {
            ...
            else
            {
                ...
                memcpy(key->pub_key, pubkey, usedlen);
                OPENSSL_free(pubkey);
...
static const OSSL_PARAM *vic408_keymgmt_import_types(int selection)
{
...
static int vic408_keymgmt_export(void *keydata, int selection, OSSL_CALLBACK *param_cb, void *cbarg)
{
    sealsq_VIC408_provider_store_obj_t *key = (sealsq_VIC408_provider_store_obj_t *)keydata;
    OSSL_PARAM params[8];
    ...
    int ret;
    sealsq_VIC408_Prov_Print(LOG_DBG_ON, "Enter - %s key index - %d", __FUNCTION__, key->privkey_index);
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
    {
        sealsq_VIC408_Prov_Print(LOG_DBG_ON, "Ask Public Key", __FUNCTION__, key->privkey_index);
...
static const OSSL_PARAM *vic408_keymgmt_export_types(int selection)
{
...
static const char *vic408_keymgmt_query_operation_name(int operation_id)
{
...
static int vic408_keymgmt_keymgmt_match(const void *keydata1, const void *keydata2, int selection)
{
    sealsq_VIC408_Prov_Print(LOG_DBG_ON, "Enter - %s ", __FUNCTION__);
    ...
    sealsq_VIC408_Prov_Print(LOG_DBG_ON, "keyid - %d vs key id %d", key->privkey_index, key2->privkey_index);

    for (int i = 0; i < 128; i++)
    {
        if (key->pub_key[i] != key2->pub_key[i])
        {
...
const OSSL_DISPATCH sealsq_VIC408_ec_keymgmt_functions[] = {
    {OSSL_FUNC_KEYMGMT_NEW, (void (*)(void))vic408_keymgmt_new},
    {OSSL_FUNC_KEYMGMT_FREE, (void (*)(void))vic408_keymgmt_free},
    {OSSL_FUNC_KEYMGMT_HAS, (void (*)(void))vic408_keymgmt_has},
    {OSSL_FUNC_KEYMGMT_IMPORT, (void (*)(void))vic408_keymgmt_import},
    {OSSL_FUNC_KEYMGMT_IMPORT_TYPES, (void (*)(void))vic408_keymgmt_import_types},
    {OSSL_FUNC_KEYMGMT_EXPORT, (void (*)(void))vic408_keymgmt_export},
    {OSSL_FUNC_KEYMGMT_EXPORT_TYPES, (void (*)(void))vic408_keymgmt_export_types},
    {OSSL_FUNC_KEYMGMT_LOAD, (void (*)(void))vic408_keymgmt_load},
    {OSSL_FUNC_KEYMGMT_QUERY_OPERATION_NAME, (void (*)(void))vic408_keymgmt_query_operation_name},
    {OSSL_FUNC_KEYMGMT_MATCH, (void (*)(void))vic408_keymgmt_keymgmt_match},
